data aids;
   input time CD4 age cigarettes drug partners depression id;
   cd4_scale=cd4/100;
...
;
run;
/*** Fitting the Random Coefficient Model ***/
proc glimmix data=aids;
  model cd4_scale=time age cigarettes drug partners depression 
                  time*age time*cigarettes / solution ddfm=kr;
  random int /subject=id;
  output out=out pred(blup)=predblup pred(noblup)=xbeta;
run;
/* Sgplot to observe the Random Coefficient Model  and also the mean model visually.*/
/* If you need to plot for specific subject THEN use
proc sgplot data=out(where=(id=10131)) noautolegend; for id=10131 OR
proc sgplot data=out noautolegend;
   where id=10131;*/
proc sgplot data=out noautolegend;
   scatter y=cd4_scale x=time / group=id;
   series y=predblup x=time / group=id;
   title 'Random Intercept Model';
run;

proc sgplot data=out noautolegend;
   scatter y=cd4_scale x=time / group=id;
   series y=xbeta x=time / group=id;
   title 'Mean Profile';
run;
title;
/*** Fitting the Random intercept and random slope ***/
proc glimmix data=aids;
   model cd4_scale=time age cigarettes drug partners depression
                   time*age time*cigarettes 
                   / solution ddfm=kr;
   random int time / type=un subject=id ;
   output out=out pred(blup)=predblup;
run;
...
run;
title;
/*** Fitting the radial smoother model***/
proc glimmix data=aids;
   model cd4_scale=time age cigarettes drug partners depression
                   time*age time*cigarettes 
                   / solution ddfm=kr;
   random time / type=rsmooth subject=id knotmethod=equal(3) knotinfo;
   output out=out pred(blup)=predblup;
run;
